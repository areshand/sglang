...
class MultiEndedAllocator(BaseTokenToKVPoolAllocator):
    ...
    def translate_kv_loc_dense(
        ...
        *,
        ...
        with record_function("MultiEndedAlloc.translate_kv_loc_dense"):
            dense_page_stride = ps * self.kernel_page_multiplier
            if ps == 1:
    ...
    def translate_write_loc_dense(
        self,
        widened_loc: torch.Tensor,
        *,
        out: Optional[torch.Tensor] = None,
    ) -> torch.Tensor:
        """Translate a WIDENED virtual WRITE loc (`out_cache_loc`) to a DENSE id.
        ...
        `loc % dcp_size`. So this resolves ownership, collapses, and translates,
        sending the ids this rank does not own to dense id 0 — the reserved
        padding sink that every MLA write kernel skips. Equivalent to
        `translate_kv_loc_dense` when dcp_size == 1.
        """
        if self.dcp_size == 1:
            return self.translate_kv_loc_dense(widened_loc, out=out)
        with record_function("MultiEndedAlloc.translate_write_loc_dense"):
            owned = (widened_loc % self.dcp_size) == self.dcp_rank
            dense = self.translate_kv_loc_dense(widened_loc // self.dcp_size)
    ...
    def _compact_pending_impl(self, freed_physical_pages: torch.Tensor) -> None:
        ...
        if src_list:
            ...
            # Expand page ids to PHYSICAL token ids for the token-granular move
            # kernel (rows in the pool, so pool_page_size, not the widened one).
...
class UnifiedMambaTokenToKVPoolAllocator(BaseTokenToKVPoolAllocator):
    ...
    def translate_write_loc_dense(
        ...
        *,
        ...
        return self.full_attn_allocator.translate_write_loc_dense(loc, out=out)
