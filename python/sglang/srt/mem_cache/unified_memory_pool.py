...
class UnifiedMLATokenToKVPool(MLATokenToKVPool):
    ...
    def set_mla_kv_buffer(
        ...
        """
        maybe_detect_oob(loc, 0, self._dense_size, "set_mla_kv_buffer (unified MLA)")
        layer_id = (
    ...
    def set_kv_buffer(
        ...
        maybe_detect_oob(loc, 0, self._dense_size, "set_kv_buffer (unified MLA)")
        layer_id = (
            layer_id_override if layer_id_override is not None else layer.layer_id
        )
        if cache_k.dtype != self.dtype:
...
def init_unified_mamba_pools(
    *,
    ...
    if use_mla_backend:
        ...
        # (eager-prefill-only paths; never captured in a cuda graph). The write
        # loc additionally carries the DCP owner rule, hence the second hook.
        token_to_kv_pool._full_translate = allocator.translate_kv_loc_dense
        token_to_kv_pool._full_write_translate = allocator.translate_write_loc_dense
