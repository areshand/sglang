...
class TestResolutionReadsTheDeclarations(CustomTestCase):
    ...
    def test_no_handler_reads_a_field_off_self(self):
        ...
        # The pipeline lives in `arg_groups/` now, so what the dispatcher
        # still reaches inside the class is these read wrappers rather than the
        # old fifty-odd handlers. The other side is covered by
        # `test_no_hook_reads_a_field_off_the_record`, which walks the package.
        # Pinned rather than counted: a walk that collapsed to the wrappers
        # would clear any floor low enough to admit them.
        self.assertEqual(
            set(handlers),
            {
    ...
    def test_no_member_recomputes_from_a_raw_field(self):
        ...
        # Dropped from >100 when the forwarding slots went: the floor is here to
        # catch the scan collapsing, not to pin the class's size.
